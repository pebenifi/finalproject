    ...
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if ('1.1' in text) or ('условия' in text):
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message='Стать участниками социальной сети МШ могут все ученики,'
                                         ' учителя, администрация. Для регистрации необходимо указать адрес электронной'
                                         ' почты МШ, фамилию, имя, телефон и придумать пароль, также можно ввести '
                                         'информацию в поле "О себе"',
                                 random_id=random.randint(0, 2 ** 64))

            elif ('1.2' in text) or ('зарегистрир' in text):
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message='Для регистрации необходимо указать адрес электронной почты, фамилию и имя с '
                                         'большой буквы в иминительном падеже, '
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message='Чат-бот ВКонтакте - это я, Иннокентий. Всегда готов прийти на помощь, '
                                         'подсказать при работе в социальной сети МШ и просто поболтать о жизни',
                                 random_id=random.randint(0, 2 ** 64))
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message='Как классно, что соц.сеть МШ интересна! Теперь можно узнать о ней все (или '
                                         'почти все)',
                                 random_id=random.randint(0, 2 ** 64))
                vk.messages.send(user_id=event.obj.message['from_id'],
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message='Вот, в чем я могу помочь относительно социальной сети МШ:'
                                         '\n1. регистрация'
...
def apivk():
    vk_session = vk_api.VkApi(
        token='2939b14eb75df1a554fb1cceeb6b069077e22485136a6ea130abe560ec46bf820c3305da93f3d8e19585d')
    longpoll = VkBotLongPoll(vk_session, 204215250)
    m = 0
    vk = vk_session.get_api()
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if m == 0:
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message=f"Приятно познакомиться, {name}. Меня зовут Иннокентий, для друзей просто "
                                         f"Кеша."
                                         f" Я проводник по миру МШ",
                                 random_id=random.randint(0, 2 ** 64))
                vk.messages.send(user_id=event.obj.message['from_id'],
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message=f'{name}, теперь вы в блоке расписание. Чтобы выйти, напишите "стоп", '
                                         f'если хотите продолжить - "ок"',
                                 random_id=random.randint(0, 2 ** 64))
                timet(vk, longpoll, vk_session)
            elif 'соц' in text:
                vk.messages.send(user_id=event.obj.message['from_id'],
                                 message=f'Теперь {name} в блоке социальной сети МШ. Чтобы выйти, напишите "стоп". '
